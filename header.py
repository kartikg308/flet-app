...
class AppHeader(UserControl):
    ...
    def app_header_search(self):
        return Container(
            ...
            content=Row(
                ...
                controls=[
                    Icon(name="SEARCH_ROUNDED", size=17, opacity=0.85),
                    TextField(border_color="transparent", height=20, text_size=14, content_padding=8,
                              cursor_color="white", cursor_width=1, color="white", hint_text="Search",
    ...
    def app_header_avatar(self):
        return Container(
            content=IconButton("PERSON")
        )
    
    def show_search_bar(self, e):
        if e.data == "true": 
            self.controls[0].data.controls[1].opacity = 1
            self.controls[0].data.controls[1].update()
        else:
            self.controls[0].data.controls[1].content.controls[1].value = ""
            self.controls[0].data.controls[1].opacity = 0
            self.controls[0].data.controls[1].update()
